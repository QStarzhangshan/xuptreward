package cn.xupt.reward.project.map.domain;

import java.security.Timestamp;
import java.util.Date;

public class TeacherProject2Map {
	private long colId;
	private String colCode;
	private String colPnum;
	private String colPname;
	private String colPtype;
	private String colPsubtype;
	private String colMtype;
	private String colContent;
	private long colPmoney;
	private String colPecent;
	private String colSign;
	private String colFac;
	private Date colMdate;
	private String colMoney1;
	private Timestamp colTime1;
	private String colXupt1;
	private String colMoney2;
	private Timestamp colTime2;
	private String colXupt2;
	private String colMoney3;
	private Timestamp colTime3;
	private String colXupt3;
	private String colIdentifytype;
	private String colSummary;
	private String colInnovation;
	private String colExpected;
	private String colTarget;
	private String colTextent;
	private String colSchedule;
	private String colMember;
	private Date colBdate;
	private Date colEdate;
	private long colUorder;
	private String colAuthorlist;
	private String colFactor;
	private String colUnitlist;
	private long colNumber;
	private long colOrder;
	private long colXiaoneinumber;
	private long colXiaoneiorder;
	private Date colDate;
	private Date colTtime;
	private String colAuditor1;
	private String colDepartment1;
	private Timestamp colAtime1;
	private String colStatus1;
	private String colAtxt1;
	private String colAuditor2;
	private String colDepartment2;
	private Timestamp colAtime2;
	private String colStatus2;
	private String colAtxt2;
	private String colAuditor3;
	private String colDepartment3;
	private Timestamp colAtime3;
	private String colStatus3;
	private String colAtxt3;
	private String colEvidence;
	private String colImageurl;
	private String colMoney;
	private String colNote;
	public long getColId() {
		return colId;
	}
	public void setColId(long colId) {
		this.colId = colId;
	}
	public String getColCode() {
		return colCode;
	}
	public void setColCode(String colCode) {
		this.colCode = colCode;
	}
	public String getColPnum() {
		return colPnum;
	}
	public void setColPnum(String colPnum) {
		this.colPnum = colPnum;
	}
	public String getColPname() {
		return colPname;
	}
	public void setColPname(String colPname) {
		this.colPname = colPname;
	}
	public String getColPtype() {
		return colPtype;
	}
	public void setColPtype(String colPtype) {
		this.colPtype = colPtype;
	}
	public String getColPsubtype() {
		return colPsubtype;
	}
	public void setColPsubtype(String colPsubtype) {
		this.colPsubtype = colPsubtype;
	}
	public String getColMtype() {
		return colMtype;
	}
	public void setColMtype(String colMtype) {
		this.colMtype = colMtype;
	}
	public String getColContent() {
		return colContent;
	}
	public void setColContent(String colContent) {
		this.colContent = colContent;
	}
	public long getColPmoney() {
		return colPmoney;
	}
	public void setColPmoney(long colPmoney) {
		this.colPmoney = colPmoney;
	}
	public String getColPecent() {
		return colPecent;
	}
	public void setColPecent(String colPecent) {
		this.colPecent = colPecent;
	}
	public String getColSign() {
		return colSign;
	}
	public void setColSign(String colSign) {
		this.colSign = colSign;
	}
	public String getColFac() {
		return colFac;
	}
	public void setColFac(String colFac) {
		this.colFac = colFac;
	}
	public Date getColMdate() {
		return colMdate;
	}
	public void setColMdate(Date colMdate) {
		this.colMdate = colMdate;
	}
	public String getColMoney1() {
		return colMoney1;
	}
	public void setColMoney1(String colMoney1) {
		this.colMoney1 = colMoney1;
	}
	public Timestamp getColTime1() {
		return colTime1;
	}
	public void setColTime1(Timestamp colTime1) {
		this.colTime1 = colTime1;
	}
	public String getColXupt1() {
		return colXupt1;
	}
	public void setColXupt1(String colXupt1) {
		this.colXupt1 = colXupt1;
	}
	public String getColMoney2() {
		return colMoney2;
	}
	public void setColMoney2(String colMoney2) {
		this.colMoney2 = colMoney2;
	}
	public Timestamp getColTime2() {
		return colTime2;
	}
	public void setColTime2(Timestamp colTime2) {
		this.colTime2 = colTime2;
	}
	public String getColXupt2() {
		return colXupt2;
	}
	public void setColXupt2(String colXupt2) {
		this.colXupt2 = colXupt2;
	}
	public String getColMoney3() {
		return colMoney3;
	}
	public void setColMoney3(String colMoney3) {
		this.colMoney3 = colMoney3;
	}
	public Timestamp getColTime3() {
		return colTime3;
	}
	public void setColTime3(Timestamp colTime3) {
		this.colTime3 = colTime3;
	}
	public String getColXupt3() {
		return colXupt3;
	}
	public void setColXupt3(String colXupt3) {
		this.colXupt3 = colXupt3;
	}
	public String getColIdentifytype() {
		return colIdentifytype;
	}
	public void setColIdentifytype(String colIdentifytype) {
		this.colIdentifytype = colIdentifytype;
	}
	public String getColSummary() {
		return colSummary;
	}
	public void setColSummary(String colSummary) {
		this.colSummary = colSummary;
	}
	public String getColInnovation() {
		return colInnovation;
	}
	public void setColInnovation(String colInnovation) {
		this.colInnovation = colInnovation;
	}
	public String getColExpected() {
		return colExpected;
	}
	public void setColExpected(String colExpected) {
		this.colExpected = colExpected;
	}
	public String getColTarget() {
		return colTarget;
	}
	public void setColTarget(String colTarget) {
		this.colTarget = colTarget;
	}
	public String getColTextent() {
		return colTextent;
	}
	public void setColTextent(String colTextent) {
		this.colTextent = colTextent;
	}
	public String getColSchedule() {
		return colSchedule;
	}
	public void setColSchedule(String colSchedule) {
		this.colSchedule = colSchedule;
	}
	public String getColMember() {
		return colMember;
	}
	public void setColMember(String colMember) {
		this.colMember = colMember;
	}
	public Date getColBdate() {
		return colBdate;
	}
	public void setColBdate(Date colBdate) {
		this.colBdate = colBdate;
	}
	public Date getColEdate() {
		return colEdate;
	}
	public void setColEdate(Date colEdate) {
		this.colEdate = colEdate;
	}
	public long getColUorder() {
		return colUorder;
	}
	public void setColUorder(long colUorder) {
		this.colUorder = colUorder;
	}
	public String getColAuthorlist() {
		return colAuthorlist;
	}
	public void setColAuthorlist(String colAuthorlist) {
		this.colAuthorlist = colAuthorlist;
	}
	public String getColFactor() {
		return colFactor;
	}
	public void setColFactor(String colFactor) {
		this.colFactor = colFactor;
	}
	public String getColUnitlist() {
		return colUnitlist;
	}
	public void setColUnitlist(String colUnitlist) {
		this.colUnitlist = colUnitlist;
	}
	public long getColNumber() {
		return colNumber;
	}
	public void setColNumber(long colNumber) {
		this.colNumber = colNumber;
	}
	public long getColOrder() {
		return colOrder;
	}
	public void setColOrder(long colOrder) {
		this.colOrder = colOrder;
	}
	public long getColXiaoneinumber() {
		return colXiaoneinumber;
	}
	public void setColXiaoneinumber(long colXiaoneinumber) {
		this.colXiaoneinumber = colXiaoneinumber;
	}
	public long getColXiaoneiorder() {
		return colXiaoneiorder;
	}
	public void setColXiaoneiorder(long colXiaoneiorder) {
		this.colXiaoneiorder = colXiaoneiorder;
	}
	public Date getColDate() {
		return colDate;
	}
	public void setColDate(Date colDate) {
		this.colDate = colDate;
	}
	public Date getColTtime() {
		return colTtime;
	}
	public void setColTtime(Date colTtime) {
		this.colTtime = colTtime;
	}
	public String getColAuditor1() {
		return colAuditor1;
	}
	public void setColAuditor1(String colAuditor1) {
		this.colAuditor1 = colAuditor1;
	}
	public String getColDepartment1() {
		return colDepartment1;
	}
	public void setColDepartment1(String colDepartment1) {
		this.colDepartment1 = colDepartment1;
	}
	public Timestamp getColAtime1() {
		return colAtime1;
	}
	public void setColAtime1(Timestamp colAtime1) {
		this.colAtime1 = colAtime1;
	}
	public String getColStatus1() {
		return colStatus1;
	}
	public void setColStatus1(String colStatus1) {
		this.colStatus1 = colStatus1;
	}
	public String getColAtxt1() {
		return colAtxt1;
	}
	public void setColAtxt1(String colAtxt1) {
		this.colAtxt1 = colAtxt1;
	}
	public String getColAuditor2() {
		return colAuditor2;
	}
	public void setColAuditor2(String colAuditor2) {
		this.colAuditor2 = colAuditor2;
	}
	public String getColDepartment2() {
		return colDepartment2;
	}
	public void setColDepartment2(String colDepartment2) {
		this.colDepartment2 = colDepartment2;
	}
	public Timestamp getColAtime2() {
		return colAtime2;
	}
	public void setColAtime2(Timestamp colAtime2) {
		this.colAtime2 = colAtime2;
	}
	public String getColStatus2() {
		return colStatus2;
	}
	public void setColStatus2(String colStatus2) {
		this.colStatus2 = colStatus2;
	}
	public String getColAtxt2() {
		return colAtxt2;
	}
	public void setColAtxt2(String colAtxt2) {
		this.colAtxt2 = colAtxt2;
	}
	public String getColAuditor3() {
		return colAuditor3;
	}
	public void setColAuditor3(String colAuditor3) {
		this.colAuditor3 = colAuditor3;
	}
	public String getColDepartment3() {
		return colDepartment3;
	}
	public void setColDepartment3(String colDepartment3) {
		this.colDepartment3 = colDepartment3;
	}
	public Timestamp getColAtime3() {
		return colAtime3;
	}
	public void setColAtime3(Timestamp colAtime3) {
		this.colAtime3 = colAtime3;
	}
	public String getColStatus3() {
		return colStatus3;
	}
	public void setColStatus3(String colStatus3) {
		this.colStatus3 = colStatus3;
	}
	public String getColAtxt3() {
		return colAtxt3;
	}
	public void setColAtxt3(String colAtxt3) {
		this.colAtxt3 = colAtxt3;
	}
	public String getColEvidence() {
		return colEvidence;
	}
	public void setColEvidence(String colEvidence) {
		this.colEvidence = colEvidence;
	}
	public String getColImageurl() {
		return colImageurl;
	}
	public void setColImageurl(String colImageurl) {
		this.colImageurl = colImageurl;
	}
	public String getColMoney() {
		return colMoney;
	}
	public void setColMoney(String colMoney) {
		this.colMoney = colMoney;
	}
	public String getColNote() {
		return colNote;
	}
	public void setColNote(String colNote) {
		this.colNote = colNote;
	}
	@Override
	public String toString() {
		return "TeacherProject2Map [colId=" + colId + ", colCode=" + colCode + ", colPnum=" + colPnum + ", colPname="
				+ colPname + ", colPtype=" + colPtype + ", colPsubtype=" + colPsubtype + ", colMtype=" + colMtype
				+ ", colContent=" + colContent + ", colPmoney=" + colPmoney + ", colPecent=" + colPecent + ", colSign="
				+ colSign + ", colFac=" + colFac + ", colMdate=" + colMdate + ", colMoney1=" + colMoney1 + ", colTime1="
				+ colTime1 + ", colXupt1=" + colXupt1 + ", colMoney2=" + colMoney2 + ", colTime2=" + colTime2
				+ ", colXupt2=" + colXupt2 + ", colMoney3=" + colMoney3 + ", colTime3=" + colTime3 + ", colXupt3="
				+ colXupt3 + ", colIdentifytype=" + colIdentifytype + ", colSummary=" + colSummary + ", colInnovation="
				+ colInnovation + ", colExpected=" + colExpected + ", colTarget=" + colTarget + ", colTextent="
				+ colTextent + ", colSchedule=" + colSchedule + ", colMember=" + colMember + ", colBdate=" + colBdate
				+ ", colEdate=" + colEdate + ", colUorder=" + colUorder + ", colAuthorlist=" + colAuthorlist
				+ ", colFactor=" + colFactor + ", colUnitlist=" + colUnitlist + ", colNumber=" + colNumber
				+ ", colOrder=" + colOrder + ", colXiaoneinumber=" + colXiaoneinumber + ", colXiaoneiorder="
				+ colXiaoneiorder + ", colDate=" + colDate + ", colTtime=" + colTtime + ", colAuditor1=" + colAuditor1
				+ ", colDepartment1=" + colDepartment1 + ", colAtime1=" + colAtime1 + ", colStatus1=" + colStatus1
				+ ", colAtxt1=" + colAtxt1 + ", colAuditor2=" + colAuditor2 + ", colDepartment2=" + colDepartment2
				+ ", colAtime2=" + colAtime2 + ", colStatus2=" + colStatus2 + ", colAtxt2=" + colAtxt2
				+ ", colAuditor3=" + colAuditor3 + ", colDepartment3=" + colDepartment3 + ", colAtime3=" + colAtime3
				+ ", colStatus3=" + colStatus3 + ", colAtxt3=" + colAtxt3 + ", colEvidence=" + colEvidence
				+ ", colImageurl=" + colImageurl + ", colMoney=" + colMoney + ", colNote=" + colNote + "]";
	}
	
	
	
	
	
}
